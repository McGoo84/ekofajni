...
include_once('tbcmsfooterlogo_image_resize.class.php');
// Image_upload class is use for image uploading
class TbcmsFooterLogoImageUpload extends Module
{
    public function imageUploading($image_src_1, $old_file)
    {
        ...
        $width = 50;
        $height = 50;
        $resize_type = 4;//[options: 1 (exact), 2 (portrait), 3 (landscape), 4 (auto), 5 (crop)]

        /************resize settings***************/
        $savePath = _PS_MODULE_DIR_.'tbcmsfooterlogo/views/img/';
        ...
        if ($resultType) {
            ...
            if ($resultUpload) {//success
                $this->resizeObj = new TbcmsFooterLogoResize($save_destination);
                // Resize image
                $this->resizeObj->resizeImage($width, $height, $resize_type);
                // Save image
                $new_img = $savePath.$img_tiny.$imgName;
                $this->resizeObj->saveImage($new_img, false, 10);
                ...
                if (@file_exists(dirname(__FILE__).'./../views/img/'.$old_file)
                    ...
                    @unlink(dirname(__FILE__).'./../views/img/tiny/'.$old_file);
                }
                $successUpload = true;
            } else {
