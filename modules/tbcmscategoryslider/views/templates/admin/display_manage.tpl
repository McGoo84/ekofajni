...
<div class="panel">
    ...
    <table id="table-data" class="table tbcmstable tbcmscategoryslider">
        <thead>
            <tr class="nodrag nodrop">
                <th class="fixed-width-xs center">
                    <span class="title_box active">{l s='Category Images' mod='tbcmscategoryslider'}</span>
                </th>
                {if $show_fields.category_title == true}
                <th class="fixed-width-xs center">
                    <span class="title_box active">{l s='Category Name' mod='tbcmscategoryslider'}</span>
                </th>
                {/if}
                
                {if $show_fields.title == true}
                <th class="fixed-width-xs center">
                    <span class="title_box active">{l s='Category Custom Name' mod='tbcmscategoryslider'}</span>
                </th>
                {/if}

                {if $show_fields.short_description == true}
                <th class="fixed-width-xs center">
                    <span class="title_box active">{l s='Category Description' mod='tbcmscategoryslider'}</span>
                </th>
                {/if}

                <th class="fixed-width-xs center status">
                    <span class="title_box active">{l s='Status' mod='tbcmscategoryslider'}</span>
                </th>
                <th class="fixed-width-xs center action">
                    <span class="title_box active">{l s='Action' mod='tbcmscategoryslider'}</span>
                </th>
            </tr>   
        </thead>
                ...
                <tr id="recordsArray_{$Data.id|escape:'htmlall':'UTF-8'}">
                    ...
                    <td class="pointer fixed-width-xs center">
                        <form action="" class="actionEdit" name="actionEdit" method="post">
                            ...
                            <button title="Edit" class="edit" onclick="$(this).parent('.actionEdit').trigger('submit');" ><i class="icon-pencil"></i> {l s='Edit' mod='tbcmscategoryslider'} </button>
                        </form>
                        <form  action="" class="actionDelete" name="actionDelete" method="post">
                            <input type="hidden" name="id" value="{$Data.id|escape:'html':'UTF-8'}">
                            <input type="hidden" name="action" value="remove">
                            <button title="Delete" class="delete tbcmsdelete"><i class='icon-trash'></i> {l s='Delete' mod='tbcmscategoryslider'} </button>
                        </form>
                    </td>
                </tr>
            {/foreach}
